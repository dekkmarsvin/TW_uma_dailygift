...
async function waitForPageReady(page) {
    ...
    try {
        ...
        console.warn('Main page markers not found within 15 seconds. Continuing with DOM inspection.');
    }
...
async function testLotteryFeature() {
    ...
    try {
        ...
        console.log(`  Logged Out: ${loginInfo.loggedOut ? 'YES' : 'NO'}\n`);
        ...
        if (prizeInfo.prizes.length > 0) {
            prizeInfo.prizes.forEach((prize, index) => {
                console.log(`\nPrize ${index + 1}:`);
                console.log(`  Name: ${prize.isGrandPrize ? '[special prize] ' : ''}${prize.name}`);
            ...
            console.log('  No prize elements found with stock information');
        }
